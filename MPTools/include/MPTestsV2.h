...
//#define MAX 256
extern string sreal_result, sexpression, sdue_result, smessage, sbadvalue, sexception;
extern bool bexception;
//    char csreal_result[MAX], csdue_result[MAX],csrandom[MAX];
//    char creal_result, cdue_result, crandom;
//    int ireal_result, idue_result;
//     double dreal_result, ddue_result;
...
#define DEF_CONTEXT(...) sexpression=#__VA_ARGS__; \
__VA_ARGS__
#define OBTAINED_STRING sreal_result
#define EXPECTED_STRING sdue_result
#define TEST_OUTCOME(EXPR) EXPECTED_STRING=EXPR
#ifdef __REPORT_ALL_TESTS__
#define TEST_EXPRESSION(EXPR) sexpression+=#EXPR;\
    OBTAINED_STRING="???"
#else
#define TEST_EXPRESSION(EXPR) sexpression+=#EXPR;\
try {\
...
#endif
#define OTHERWISE(EXPR) smessage=EXPR
#define OUTCOME (!bexception? std::string("")\
+sexpression+" should give \""+EXPECTED_STRING+"\" but gives \""+ OBTAINED_STRING+"\"":\
...
#define EXPECT_STRING_EQ \
    UPDATEIDTEST();\
    IDTEST(idtest);\
    if (!bexception) {\
    REPORT_UNIT_TEST(OUTCOME,OBTAINED_STRING == EXPECTED_STRING)\
    ASSERT_STREQ(EXPECTED_STRING.c_str(),OBTAINED_STRING.c_str()) << OUTCOME << endl;\
    } else {\
    REPORT_UNIT_TEST("Unexpected "+OUTCOME,false)\
    ASSERT_FALSE(bexception) << "Unexpected "+OUTCOME << endl;\
    }
...
#define EXPECT_EXCEPTION \
    ...
    IDTEST(idtest);\
    if (!bexception) {\
    REPORT_UNIT_TEST(string("Should THROW an exception"),false)\
    ASSERT_TRUE(bexception) << string("Should THROW an exception")<< endl;\
    } else {\
     REPORT_UNIT_TEST(OUTCOME,true)\
     ASSERT_TRUE(bexception) << endl;\
    }
...
#define REPORT_UNIT_TEST_EXCEPTION(MESSAGE, ...) \
    do {\
    std::string RTHead ="";\
    UPDATEIDTEST();\
    IDTEST(idtest);\
